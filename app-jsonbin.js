// Hybrid Voting App with JSONBin.io Support
// Works on GitHub Pages with shared voting across all users!
...
async function checkBackendAvailability() {
    ...
    try {
        ...
        console.log('Backend not available, checking JSONBin mode');
        return false;
    }
}
...
async function createJSONBinStorage(profile) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log(`Created JSONBin for ${profile}:`, data.metadata.id);
...
async function loadConfig() {
    ...
    } else if (isJSONBinMode && APP_CONFIG.JSONBIN_MODE) {
        // Load from JSONBin
...
function startJSONBinPolling() {
    if (!isJSONBinMode || !currentBinId) return;
...
async function vote(item) {
    ...
    } else if (isJSONBinMode && currentBinId) {
        // JSONBin mode
        const binData = await loadJSONBinData(currentBinId);
        if (binData) {
            // Update votes
